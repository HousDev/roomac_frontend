...
export default function PropertyListClient({ initialProperties }: PropertyListClientProps) {
  ...
  }, [properties, searchQuery, statusFilter, tagFilter]); // REMOVE filteredProperties from dependencies
...
const handleFormSubmit = useCallback(async (
  ...
  try {
    ...
    console.log("🔍 DEBUG: Starting form submit");
    console.log("📝 Edit Mode:", editMode);
    console.log("🆔 Selected Property ID:", selectedProperty?.id);
    ...
    if (editMode && selectedProperty) {
      console.log(`🔄 Attempting UPDATE for property ID: ${selectedProperty.id}`);
      
      // DEBUG: First check if property exists
      try {
        const checkRes = await getProperty(selectedProperty.id);
        console.log("✅ Property exists check:", checkRes);
      } catch (checkError) {
      ...
      console.log("➕ Attempting CREATE new property");
  ...
  return (
    <div >
      <Card className="border-0 shadow-xl bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 backdrop-blur-xl overflow-hidden ">
        {/* Header Component */}
        <PropertyHeader
        ...
        {/* Content */}
        <div className="max-h-[calc(100vh-160px)] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-blue-100">
          <CardContent className="p-4">
