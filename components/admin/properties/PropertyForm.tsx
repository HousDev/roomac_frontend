...
// // ✅ MultiSelect is now OUTSIDE PropertyForm — this is the key fix!
// // When defined inside, React destroys & recreates it on every state change,
// // which resets the open state and closes the dropdown on every click.
// const MultiSelect = ({
//   options,
//   selected,
//   onChange,
...
//         className={`min-h-8 w-full px-3 py-1.5 border rounded-lg text-[10px] md:text-xs flex items-center cursor-pointer select-none bg-white ${disabled ? "opacity-50 cursor-not-allowed" : "hover:border-blue-400"}`}
//       >
//         <span className="text-gray-400 flex-1">
//           {selected.length > 0 ? `${selected.length} selected` : placeholder}
//         </span>
//         <svg className={`h-3 w-3 text-gray-400 transition-transform ${open ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor">
//           <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" />
//         </svg>
//       </div>

//       {/* Dropdown — plain div, no shadcn, no auto-close */}
//       {open && (
//         <div className="absolute z-[99999] top-full left-0 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-52 overflow-y-auto">
//           {disabled ? (
//             <div className="px-3 py-2 text-[10px] text-gray-500 flex items-center gap-1">
...
//                   className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer select-none transition-colors ${isSelected ? "bg-blue-50" : "hover:bg-gray-50"}`}
//                 >
...
//                   </div>
//                   <span className={`text-[10px] md:text-xs ${isSelected ? "text-blue-700 font-medium" : "text-gray-700"}`}>
//                     {option.name}
//                   </span>
//                 </div>
//               );
//             })
//           ) : (
//             <div className="px-3 py-2 text-[10px] text-gray-500">No options available</div>
//           )}
//         </div>
//       )}
...
//           {selected.map((value) => (
//             <span key={value} className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-[9px] md:text-[10px] flex items-center gap-0.5">
//               {value}
...
//       try {
//         parsedTags = JSON.parse(selectedProperty.tags);
//       } catch {
//         parsedTags = Array.isArray(selectedProperty.tags)
//           ? selectedProperty.tags
//           : selectedProperty.tags ? [selectedProperty.tags] : [];
//       }
//     }

//     // Initialize form data with raw values first
//     const initialFormData = {
//       name: selectedProperty.name || "",
//       city_id: selectedProperty.city_id || "",
//       state: selectedProperty.state || "", // This might be an ID
//       area: selectedProperty.area || "",
//       address: selectedProperty.address || "",
//       total_rooms: selectedProperty.total_rooms || 0,
//       total_beds: selectedProperty.total_beds || 0,
//       floor: selectedProperty.floor || "", // This might be an ID
...
//     // Set the selected rules and terms from the parsed values
//     setSelectedRules(parsedPropertyRules);
//     setSelectedAdditionalTerms(parsedAdditionalTerms);
//     setSelectedTags(parsedTags);
    
//     setExistingPhotoUrls(uniquePhotoUrls);
//     setPhotoFiles([]);
//     setRemovedPhotoUrls([]);
...
// // Update period values when masters load - this is the key fix!
// // Update values when masters load - including state and floor
// useEffect(() => {
...
//       // Update state - match by ID
//       if (selectedProperty.state) {
//         const stateId = String(selectedProperty.state);
//         const matchingState = commonMasters["States"]?.find(state => 
//           String(state.id) === stateId || state.name === stateId
//         );
        
//         if (matchingState) {
//           updates.state = String(matchingState.id); // Store the ID, not the name
//         }
//       }
      
//       // Update floor - match by ID or name
//       if (selectedProperty.floor) {
//         const floorValue = String(selectedProperty.floor);
//         const matchingFloor = propertiesMasters["Floors"]?.find(floor => 
//           String(floor.id) === floorValue || floor.name === floorValue
//         );
        
//         if (matchingFloor) {
//           updates.floor = matchingFloor.name; // Floors use name as value in Select
//         }
//       }

//       // Update lock-in period - match by value
...
//         if (matchingOption) {
//           updates.lockin_period_months = matchingOption.name;
//         }
//       }

//       // Update notice period - match by value
...
//         if (matchingOption) {
//           updates.notice_period_days = matchingOption.name;
//         }
//       }
...
//         const matchingRule = propertiesMasters["Property Rules"].find(
//           rule => rule.id === Number(id) || rule.name === String(id)
//         );
...
//         const matchingTerm = propertiesMasters["Additional Terms"].find(
//           term => term.id === Number(id) || term.name === String(id)
//         );
...
// // Update the useEffect that loads masters when form opens
// useEffect(() => {
//   if (open) {
...
//         // If we're in edit mode and have selectedProperty, initialize the form after masters are loaded
...
// // Function to initialize form with masters data
...
//   // Remove duplicates from photo URLs
//   const uniquePhotoUrls = [...new Set(photoUrls)];
...
//     try {
//       parsedPropertyRules = JSON.parse(selectedProperty.property_rules);
//       console.log("Parsed property rules:", parsedPropertyRules);
//     } catch {
//       parsedPropertyRules = Array.isArray(selectedProperty.property_rules)
//         ? selectedProperty.property_rules
//         : selectedProperty.property_rules ? [selectedProperty.property_rules] : [];
...
//     try {
//       parsedAdditionalTerms = JSON.parse(selectedProperty.additional_terms);
//       console.log("Parsed additional terms:", parsedAdditionalTerms);
//     } catch {
//       parsedAdditionalTerms = Array.isArray(selectedProperty.additional_terms)
//         ? selectedProperty.additional_terms
//         : selectedProperty.additional_terms ? [selectedProperty.additional_terms] : [];
...
//   // Handle city - store as ID
//   let cityValue = selectedProperty.city_id || "";
  
//   // Handle state - find matching state ID
//   let stateValue = "";
//   if (selectedProperty.state && commonMasters["States"]?.length > 0) {
//     const stateId = String(selectedProperty.state);
//     // Check if it's already an ID
//     const matchingState = commonMasters["States"].find(state => 
//       String(state.id) === stateId
//     );
//     if (matchingState) {
//       stateValue = String(matchingState.id);
//       console.log("Mapped state to ID:", stateValue);
//     } else {
//       // Try to find by name
//       const stateByName = commonMasters["States"].find(state => 
//         state.name.toLowerCase() === stateId.toLowerCase()
//       );
//       if (stateByName) {
//         stateValue = String(stateByName.id);
//         console.log("Mapped state name to ID:", stateValue);
//       } else {
//         stateValue = stateId; // Keep original if no match
//       }
//     }
//   } else {
//     stateValue = selectedProperty.state || "";
//   }

//   // Handle floor - find matching floor name
//   let floorValue = "";
//   if (selectedProperty.floor && propertiesMasters["Floors"]?.length > 0) {
//     const floorStr = String(selectedProperty.floor);
//     // Try to find by ID first
//     const matchingFloor = propertiesMasters["Floors"].find(floor => 
//       String(floor.id) === floorStr
//     );
//     if (matchingFloor) {
//       floorValue = matchingFloor.name;
//       console.log("Mapped floor ID to name:", floorValue);
//     } else {
//       // If not found by ID, try by name
//       const floorByName = propertiesMasters["Floors"].find(floor => 
//         floor.name === floorStr
//       );
//       if (floorByName) {
//         floorValue = floorByName.name;
//         console.log("Floor name matched:", floorValue);
//       } else {
//         floorValue = floorStr; // Keep original if no match
//       }
//     }
//   } else {
//     floorValue = selectedProperty.floor || "";
//   }

//   // Handle lock-in period - find matching option name
//   let lockinValue = "";
...
//     const matchingOption = propertiesMasters["Lock-in Period"].find(option => {
//       const optionNum = parseInt(option.name);
//       return optionNum === lockinNum;
//     });
//     if (matchingOption) {
//       lockinValue = matchingOption.name;
//       console.log("Mapped lock-in period to:", lockinValue);
//     } else {
//       lockinValue = String(selectedProperty.lockin_period_months);
//     }
//   } else {
//     lockinValue = selectedProperty.lockin_period_months || "";
//   }

//   // Handle notice period - find matching option name
//   let noticeValue = "";
...
//     const matchingOption = propertiesMasters["Notice Period"].find(option => {
//       const optionNum = parseInt(option.name);
//       return optionNum === noticeNum;
//     });
//     if (matchingOption) {
//       noticeValue = matchingOption.name;
//       console.log("Mapped notice period to:", noticeValue);
//     } else {
//       noticeValue = String(selectedProperty.notice_period_days);
//     }
//   } else {
//     noticeValue = selectedProperty.notice_period_days || "";
//   }

//   // Map property rules IDs to names
...
//   // Map additional terms IDs to names
...
//         tag => tag.id === Number(id) || tag.name === String(id)
//       );
...
//   // Set the selected rules, terms, and tags
//   setSelectedRules(ruleNames);
//   setSelectedAdditionalTerms(termNames);
...
// // Remove the old useEffect that was initializing the form
// // Replace it with this one that waits for masters
// useEffect(() => {
//   if (editMode && selectedProperty) {
//     // If masters are already loaded, initialize immediately
//     if (Object.keys(commonMasters).length > 0 && Object.keys(propertiesMasters).length > 0) {
//       initializeFormWithMasters();
//     }
//     // Otherwise, the initialization will happen in the loadData useEffect after masters are loaded
//   } else {
//     resetForm();
//   }
...
//   // Map selected rule names to their IDs if they exist in masters
...
//    // Map selected tag names to their IDs
...
//   const submitData = {
//     ...formData,
//     property_rules: ruleIds, // Send IDs instead of names
//     tags: tagIds, // Send tag IDs
//     additional_terms: termIds, // Send IDs instead of names
...
//           Property Tags
//         </Label>
//         <p className="text-[8px] md:text-[9px] text-gray-500 mb-2">Select tags to categorize this property (e.g., Luxury, Budget, Family, Student, etc.)</p>
//         <MultiSelect
//           options={propertiesMasters["Tags"] || []}
...
//                                 />
//                               ) : (
//                                <div className="w-5 h-5 rounded-full bg-blue-200 flex items-center justify-center flex-shrink-0 text-[9px] font-semibold text-blue-700">
//   {selectedStaff?.name
//     ?.split(" ")
//     .map(n => n[0])
//     .join("")
