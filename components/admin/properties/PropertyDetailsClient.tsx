// components/admin/properties/PropertyDetailsClient.tsx - FIXED VERSION
"use client";
...
const PropertyDetailsClient = ({ initialProperty }: PropertyDetailsClientProps) => {
    ...
    // FIX: Add safe property access with default values
    const occupiedBeds = property.occupied_beds || 0;
    const totalBeds = property.total_beds || 1; // Use 1 to avoid division by zero
    ...
    const currentPhotoUrl = property.photo_urls && property.photo_urls[currentImageIndex] 
        ? `${process.env.NEXT_PUBLIC_API_URL || ''}${property.photo_urls[currentImageIndex]}`
        : '';
                                ...
                                {property.photo_urls && property.photo_urls.length > 0 ? (
                                    <>
                                        <img
                                            ...
                                            onError={(e) => {
                                                (e.target as HTMLImageElement).src = `https://via.placeholder.com/800x450?text=${encodeURIComponent(property.name)}`;
                                            }}
                                        />
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                        ...
                                        <p className="text-slate-500 mt-4">No images available</p>
                                    </div>
                                )}
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 lg:p-8">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                                <div className="bg-blue-50 rounded-xl p-4 text-center">
                                    ...
                                    <p className="text-2xl font-bold text-slate-800">{property.total_rooms || 0}</p>
                                    <p className="text-sm text-slate-600">Total Rooms</p>
                                </div>
                                <div className="bg-purple-50 rounded-xl p-4 text-center">
                                    <Bed className="h-8 w-8 text-purple-600 mx-auto mb-2" />
                                    <p className="text-2xl font-bold text-slate-800">{property.total_beds || 0}</p>
                                ...
                                <div className="bg-orange-50 rounded-xl p-4 text-center">
                                    ...
                                    <p className="text-2xl font-bold text-slate-800">{occupiedBeds}</p>
