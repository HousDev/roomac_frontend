...
const getCityName = (
  ...
  commonMasters: Record<string, MasterValue[]>,
): string => {
...
const parseTermsWithHeaders = (
  termsString: string | null | undefined,
...
const getTermHeaderIcon = (header: string) => {
  ...
  if (header.includes("Security Deposit"))
  ...
  if (header.includes("Property Rules"))
    return <BookOpen className="h-4 w-4" />;
  if (header.includes("Rent Includes"))
...
const getTermHeaderColor = (header: string) => {
  ...
  if (header.includes("Management"))
    return "border-indigo-500 bg-indigo-50 text-indigo-800";
  if (header.includes("Taxes"))
    return "border-gray-500 bg-gray-50 text-gray-800";
  if (header.includes("Custom"))
...
const PropertyDetailsClient = ({
  ...
  // Masters data state
  const [propertiesMasters, setPropertiesMasters] = useState<
  ...
  useEffect(() => {
    const loadAllMasters = async () => {
      setLoadingMasters(true);
      try {
        await Promise.all([fetchPropertiesMasters(), fetchCommonMasters()]);
      } catch (error) {
  ...
  useEffect(() => {
    if (property) {
      ...
      if (mastersLoaded) {
        setPropertyRules(
          mapIdsToNames(rawRules, propertiesMasters, "Property Rules"),
        );
        setAdditionalTerms(
          mapIdsToNames(
            rawAdditionalTerms,
        ...
        const mappedTagNames = mapIdsToNames(
          rawTags,
          propertiesMasters,
          "Tags",
        );
    ...
    try {
      ...
      if (res && res.success && res.data) {
        const propertyData = {
          ...
          property_manager_phone_country_code:
            res.data.property_manager_phone_country_code || "", // ADD THIS LINE
          ...
          photo_urls: Array.isArray(res.data.photo_urls)
            ? res.data.photo_urls
            : [],
  ...
  const currentPhotoUrl =
    property.photo_urls && property.photo_urls[currentImageIndex]
  ...
  const getAmenityIcon = (amenity: string) => {
    ...
    // Power & Security
    if (lower.includes("power backup") || lower.includes("generator"))
    ...
    if (lower.includes("garden") || lower.includes("terrace"))
      return <Sprout className="h-5 w-5" />;

    // Appliances
    if (lower.includes("refrigerator") || lower.includes("fridge"))
      return <Refrigerator className="h-5 w-5" />;
    if (lower.includes("microwave")) return <Microwave className="h-5 w-5" />;

    // Furniture
    if (lower.includes("furnished")) return <Sofa className="h-5 w-5" />;
  ...
  // Helper function to get icon for services based on text
  const getServiceIcon = (service: string) => {
    ...
    if (lower.includes("cleaning") || lower.includes("housekeeping"))
      return <Brush className="h-4 w-4" />;
    if (lower.includes("laundry"))
      return <WashingMachine className="h-4 w-4" />;
    if (lower.includes("maintenance")) return <Wrench className="h-4 w-4" />;
    ...
    // Transportation
    if (lower.includes("transport") || lower.includes("pickup"))
      return <Car className="h-4 w-4" />;

    // Concierge & Assistance
    if (lower.includes("concierge")) return <UserCheck className="h-4 w-4" />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100">
      ...
      <div className="sticky top-20 z-10">
        <div className=" max-w-7xl mx-auto px-4 sm:px-2 lg:px-2 py-0  ">
      ...
      <div className="max-w-9xl mx-auto px-0 sm:px-2 lg:px-4 py-2 lg:py-2">
        {/* Main Grid */}
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          {/* Left Column */}
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl shadow p-2 sm:p-2">
        <div className="flex flex-wrap items-center justify-between gap-3 max-sm:flex-col max-sm:items-start max-sm:gap-1.5">
  {/* Left: Name + Location */}
  <div className="flex items-center gap-2 max-sm:gap-1.5 max-sm:w-full">
    <div className="bg-blue-100 rounded-xl p-2 max-sm:p-1">
      <Building2 className="h-5 w-5 text-blue-600 max-sm:h-3.5 max-sm:w-3.5" />
    </div>
    <div>
      <h1 className="text-xl sm:text-2xl font-bold text-slate-900 max-sm:text-base max-sm:leading-tight">
        {property.name}
      </h1>
      <div className="flex items-center gap-1 text-slate-600 mt-0.5 max-sm:mt-0">
        <MapPin className="h-3.5 w-3.5 text-blue-600 max-sm:h-2.5 max-sm:w-2.5" />
        <span className="text-sm font-medium max-sm:text-[10px] max-sm:leading-tight">
          {property.area}
          {property.city_id &&
            `, ${getCityName(property.city_id, commonMasters)}`}
        </span>
      </div>
    </div>
  </div>
  ...
  {/* Right: Tags */}
  {mappedTags.length > 0 && (
    <div className="flex flex-wrap gap-1.5 max-sm:flex-nowrap max-sm:overflow-x-auto max-sm:w-full max-sm:pb-0.5 max-sm:gap-1">
      {mappedTags.map((tag, idx) => (
        <span
          key={idx}
          className="px-3 py-1.5 bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-700 rounded-full text-xs font-medium border border-blue-200 flex items-center gap-1.5 max-sm:px-1.5 max-sm:py-0.5 max-sm:text-[9px] max-sm:gap-1 max-sm:whitespace-nowrap max-sm:flex-shrink-0"
        >
          <Tag className="h-3 w-3 max-sm:h-2 max-sm:w-2" />
          {tag}
        </span>
      ))}
    </div>
            ...
            {/* Property Info Card */}
            <div className="bg-white rounded-2xl shadow-lg p-2 sm:p-2">
  ...
  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-2 sm:p-3 text-center hover:scale-105 transition-transform">
    <DoorOpen className="h-4 w-4 sm:h-6 sm:w-6 text-blue-600 mx-auto mb-0.5 sm:mb-1" />
  ...
  <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-2 sm:p-3 text-center hover:scale-105 transition-transform">
    <Bed className="h-4 w-4 sm:h-6 sm:w-6 text-purple-600 mx-auto mb-0.5 sm:mb-1" />
    <p className="text-base sm:text-xl font-bold text-slate-900">
  ...
  <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl p-2 sm:p-3 text-center hover:scale-105 transition-transform">
    <CheckCircle2 className="h-4 w-4 sm:h-6 sm:w-6 text-green-600 mx-auto mb-0.5 sm:mb-1" />
    <p className="text-base sm:text-xl font-bold text-slate-900">
      {availableBeds}
    </p>
    <p className="text-[8px] sm:text-[10px] text-slate-600 font-medium">
      Available
          ...
          <div className="space-y-4">
                ...
                <div className="relative">
                  {getManagerPhoto() ? (
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(getManagerDisplayName())}&background=0249a8&color=fff&size=64`;
                      }}
                    />
                  ) : (
              ...
              <div className="space-y-2.5">
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2.5 border border-white/15">
                  ...
                  <div className="flex items-end gap-1">
                    ...
                    <span className="text-blue-100 text-xs mb-0.5">/month</span>
                  </div>
                </div>
            ...
            {(property.lockin_period_months &&
              property.lockin_period_months > 0) ||
                ...
                <div className="grid grid-cols-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                  {property.amenities.map((amenity, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-10 h-10 ${colorClass} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-200`}
                        >
