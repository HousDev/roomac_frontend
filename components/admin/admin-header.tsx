...
// import { Bell, LogOut, User, Home, Settings, X, Check, AlertCircle, Wrench, Calendar, Move, MessageSquare, Loader2, RefreshCw, ExternalLink } from 'lucide-react';
...
//   // Handle notification click - FIXED NAVIGATION
...
//     // Navigate to specific request page based on request_type
//     if (notification.related_entity_type === 'tenant_request' && notification.related_entity_id) {
//       const requestId = notification.related_entity_id;
//       const requestType = notification.request_type || 'general';
...
//           break;
//         case 'vacate_bed':
//           // Navigate to vacate request details page
//           router.push(`/admin/notifications`);
//           break;
//         case 'change_bed':
//           // Navigate to bed change request page
//           router.push(`/admin/notifications`);
//           break;
//         default:
//           // Fallback to complaints page
//           router.push(`/admin/notifications`);
//       }
//       setDropdownOpen(false);
//     } else {
//       // If no specific entity, go to notifications page
//       router.push('/admin/notifications');
//       setDropdownOpen(false);
//     }
//   };

//   // Update the handleNotificationClick function in admin-header.tsx:
// // const handleNotificationClick = (notification: Notification) => {
// //   // Mark as read if unread
// //   if (!notification.is_read) {
// //     handleMarkAsRead(notification.id);
...
// // Add this helper function in admin-header.tsx:
...
//   switch (request_type) {
//     case 'complaint':
//       return `/admin/complaints${related_entity_id ? `/${related_entity_id}` : ''}`;
//     case 'maintenance':
//       return `/admin/maintenance${related_entity_id ? `/${related_entity_id}` : ''}`;
//     case 'receipt':
//       return `/admin/receipts${related_entity_id ? `/${related_entity_id}` : ''}`;
//     case 'leave':
//       return `/admin/leave-requests${related_entity_id ? `/${related_entity_id}` : ''}`;
//     case 'vacate_bed':
//       return `/admin/vacate-requests${related_entity_id ? `/${related_entity_id}` : ''}`;
//     case 'change_bed':
//       return `/admin/change-bed-requests${related_entity_id ? `/${related_entity_id}` : ''}`;
//     default:
...
// // Remove or update the getNotificationUrl function as it's no longer needed
// // Or replace it with the new getNotificationRedirectUrl function
...
//         case 'leave':
//         case 'general':
//           return `/admin/complaints/${notification.related_entity_id}`;
//         case 'vacate_bed':
//           return `/admin/vacate-requests/${notification.related_entity_id}`;
//         case 'change_bed':
//           return `/admin/bed-change-requests/${notification.related_entity_id}`;
//         default:
//           return `/admin/complaints/${notification.related_entity_id}`;
//       }
//     }
...
//           <p className="text-sm text-slate-500">Manage your ROOMAC platform</p>
//         </div>
...
//                 variant="ghost" 
//                 size="icon" 
//                 className="relative hover:bg-slate-400 transition-colors"
//                 onClick={() => !dropdownOpen && loadAllData()} // Refresh when opening
//               >
//                 <Bell className="h-5 w-5" />
//                 {unreadCount > 0 && (
//                   <Badge 
//                     className="absolute -top-1 -right-1 h-6 w-6 flex items-center justify-center p-0 text-xs bg-red-500 text-white animate-pulse"
//                     variant="destructive"
//                   >
...
//           <DropdownMenu>
//             <DropdownMenuTrigger asChild>
//               <Button variant="ghost" className="flex items-center gap-2 h-auto p-2 hover:bg-slate-100">
//                 <Avatar className="h-8 w-8">
//                   <AvatarImage src={profileImage} alt="Admin" />
//                   <AvatarFallback className="bg-gradient-to-br from-[#004AAD] to-cyan-500 text-white font-semibold">
//                     {getInitials()}
//                   </AvatarFallback>
//                 </Avatar>
//                 <div className="hidden md:block text-left">
//                   <p className="text-sm font-medium leading-none hover:text-black">Administrator</p>
...
//                   <p className="text-xs text-slate-500 font-normal truncate">{adminEmail}</p>
//                 </div>
//               </DropdownMenuLabel>
//               <DropdownMenuSeparator />
