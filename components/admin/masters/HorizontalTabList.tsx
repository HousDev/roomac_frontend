...
export default function HorizontalTabList({
  ...
  onSectionClick,
    ...
    console.log("HorizontalTabList render:", { activeTab, activeSectionName, onSectionClick: !!onSectionClick });
        ...
        <div className="flex items-center p-1 gap-1 min-w-max">
          ...
          {STATIC_TABS.map((tab) => (
            <button
              ...
              `}
            >
              <span className={activeTab === tab.name && activeSectionName === "" ? "text-white" : "text-gray-500"}>
                {getTabIcon(tab.name)}
              </span>
              <span>{tab.name}</span>
              {itemCounts[tab.name] > 0 && (
          ...
          {/* Category Mapping — hardcoded, sirf onSectionClick call karta hai */}
          <button
            onClick={() => onSectionClick?.("Category Mapping")}
            className={`
              flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium whitespace-nowrap transition-colors
