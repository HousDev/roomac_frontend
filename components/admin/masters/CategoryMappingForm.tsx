...
interface Props {
  initialData?: any | null;
  childLabel?: string; // "Sub Category" (expense) or "Item" (inventory)
  onSubmit: (data: any) => Promise<void>;
  onClose: () => void;
}

export default function CategoryMappingForm({ initialData, childLabel = "Sub Category", onSubmit, onClose }: Props) {
  const [selectedCategory, setSelectedCategory] = useState<string>("");
  const [selectedSubcategories, setSelectedSubcategories] = useState<SubcategoryEntry[]>([]);
  const [submitting, setSubmitting] = useState(false);
 ...
 const addSubcategory = () => {
    const prefix = (selectedCategory || "item").replace(/\s+/g, "_");
    setSelectedSubcategories(prev => [
      ...prev,
      { subcategory_id: `${prefix}_${prev.length}_${Date.now()}`, subcategory_name: "" }
    ]);
  };
  ...
  const handleSubmit = async () => {
    if (!selectedCategory.trim()) { alert("Please enter a category name"); return; }
    const valid = selectedSubcategories.filter(s => s.subcategory_name.trim());
    if (valid.length === 0) { alert(`Add at least one ${childLabel.toLowerCase()}`); return; }

    setSubmitting(true);
    try {
      await onSubmit({
        category_id: initialData?.category_id || selectedCategory.trim(),
        category_name: selectedCategory.trim(),
        subcategories: valid.map(s => ({
          subcategory_id: s.subcategory_id,
          subcategory_name: s.subcategory_name.trim(),
        })),
      ...
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-gray-700">
                {childLabel}s <span className="text-red-500">*</span>
              </label>
            ...
            {selectedSubcategories.length === 0 ? (
              <div className="border-2 border-dashed border-gray-200 rounded-lg p-4 text-center">
                <p className="text-xs text-gray-400 mb-2">No {childLabel.toLowerCase()}s added yet</p>
                <button onClick={addSubcategory} className="text-xs text-blue-600 hover:underline">
                  + Add First {childLabel}
                </button>
              </div>
            ) : (
              <div className="space-y-2">
                {selectedSubcategories.map((sub, idx) => (
                  <div key={sub.subcategory_id} className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      value={sub.subcategory_name}
                      onChange={(e) => updateSubcategoryName(idx, e.target.value)}
                      placeholder={childLabel === "Item" ? "e.g. Mop, Bucket, Bulb" : "e.g. Deep Clean, Regular Clean"}
                      className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-blue-500 bg-gray-50"
                    />
                    <button
                      onClick={() => removeSubcategory(idx)}
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 font-medium mt-2"
            >
              <Plus size={12} /> Add {childLabel}
            </button>
            {selectedSubcategories.length > 0 && (
              <div className="mt-2 text-xs text-gray-400">
                {selectedSubcategories.filter(s => s.subcategory_name.trim()).length} {childLabel.toLowerCase()}s added
              </div>
            )}
          </div>
        </div>
