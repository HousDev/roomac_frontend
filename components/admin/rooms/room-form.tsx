...
export function RoomForm({
  ...
  const fetchRoomsMasters = async () => {
    console.log("Fetching rooms masters...");
  ...
  useEffect(() => {
    if (isEditMode && open && Object.keys(roomsMasters).length > 0 && formData) {
      console.log("Masters loaded, updating form with master values");
  ...
  useEffect(() => {
    ...
    if (isEditMode && open && rooms.length > 0) {
      ...
      if (editingRoom) {
        ...
        // Update form data with room data (master values will be matched when masters load)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[calc(100vw-1rem)] sm:max-w-[calc(100vw-2rem)] md:max-w-4xl lg:max-w-5xl max-h-[90vh] md:max-h-[90vh] overflow-hidden p-0 border-0 flex flex-col">
        ...
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="px-3 py-2 md:px-4 md:py-2.5 overflow-y-auto flex-1 min-h-0">
          <TabsList className="grid grid-cols-4 mb-2 md:mb-3 h-auto gap-0.5 md:gap-1 p-0.5 md:p-1 w-full">
          ...
          {/* Room Details Tab */}
          <TabsContent value="details" className="space-y-2 md:space-y-3 mt-0">
          ...
          <TabsContent value="photos" className="space-y-2 md:space-y-3 mt-0">
            {/* Photos tab content remains the same */}
            <div>
          ...
          {/* Video Tab */}
          <TabsContent value="video" className="space-y-2 md:space-y-3 mt-0">
            {/* Video tab content remains the same */}
            <div>
              <Label className="text-[10px] md:text-xs font-medium flex items-center gap-1 mb-2">
                <Video className="h-3 w-3 md:h-3.5 md:w-3.5" />
          ...
          <TabsContent value="amenities" className="space-y-2 md:space-y-3 mt-0">
            {/* Amenities tab content remains the same */}
        ...
        <div className="flex items-center justify-between p-2 md:p-2.5 mx-3 md:mx-4 bg-gray-50 rounded-lg">
          <div className="flex items-center gap-1.5">
