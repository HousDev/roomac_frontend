...
import { Input } from '@/components/ui/input';
import { Badge } from '@/components/ui/badge';
import { DoorOpen, Plus, Search, Filter, Bed, Users, Building2, Check } from 'lucide-react';
import { toast } from "sonner";
import { type RoomResponse ,processPhotoUrls, getMediaUrl} from '@/lib/roomsApi';
...
// In rooms-client.tsx, update the fetchFilteredRooms function:
...
const fetchFilteredRooms = useCallback(async (filters: FilterState) => {
  try {
    ...
    console.log('Sending filters to API:', processedFilters);
    ...
    if (result.success) {
      ...
      // Update pagination based on result
...
// In rooms-client.tsx, update:

const handleBulkActionComplete = useCallback((updatedRooms?: RoomResponse[]) => {
  if (updatedRooms) {
    // If we received updated rooms from the API, use them
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      ...
      toast.success("Room deleted successfully!");
      setRooms(prev => prev.filter(room => room.id.toString() !== id));
      // Also remove from selected rooms if present
      ...
      <Card className="border shadow-sm max-h-[80vh] overflow-y-auto relative">
        <CardHeader className="hidden sm:block bg-blue-500 border-b p-4 sticky top-0 z-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            {/* Right side: Filters + Add New Room */}
            <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
              {/* Old simple filters (optional - you can remove this if using only side filter) */}
...
/>
        ...
        <CardHeader className="block sm:hidden bg-blue-500 border-b p-3 sticky top-0 z-10 sm:hidden">
  <div className="flex flex-col items-start gap-2">
    ...
    <div className="flex items-center gap-2 w-full">
      <div className="relative flex-1">
        ...
        <Input
          placeholder="Search rooms..."
          className="pl-8 h-8 text-sm bg-white w-full"
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
        />
      </div>
    ...
    {/* Bottom Row: Bulk Actions (only when rooms are selected) */}
    {selectedRooms.length > 0 && (
      <div className="flex items-center justify-between w-full bg-blue-600/90 rounded-lg p-2">
