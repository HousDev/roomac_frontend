...
const StatusBadge = ({ status }: { status: 'available' | 'required' | 'pending' }) => {
  ...
  return (
    <Badge className={`${bg} ${text} ${border} border text-[10px] px-2 py-0.5 font-medium whitespace-nowrap`}>
...
function ExistingDocumentPreview({
  ...
  return (
    <div className={`${bgColor} rounded-xl p-3 mb-3 border overflow-hidden`}>
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        <div className="flex items-center gap-2 shrink-0">
          <Icon className={`h-4 w-4 ${iconColor} shrink-0`} />
          <span className="text-xs font-semibold text-slate-700">Current Document</span>
        </div>
        <Badge className="bg-green-50 text-green-600 border-green-200 text-[9px] shrink-0">Uploaded</Badge>
      </div>
      ...
      <div className="bg-white rounded-lg p-3 border border-slate-200">
        <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
          {fileUrl.match(/\.(jpg|jpeg|png|webp)$/i) ? (
            ...
            <div className="h-12 w-12 rounded-lg bg-[#e6f0ff] flex items-center justify-center shrink-0">
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="h-8 text-[11px] text-[#004aad] shrink-0"
            onClick={onView}
          >
...
function UploadSection({
  ...
  const SI = 'text-xs';
  const F = 'h-8 text-xs border-slate-200 focus:border-[#004aad] w-full';
...
additionalFiles.forEach(f => fd.append('additional_documents', f));
  ...
  return (
    <Card className="border border-[#004aad]/20 shadow-sm overflow-hidden w-full max-w-full">
      <div className="w-full px-4 sm:px-5 py-3 bg-gradient-to-r from-[#004aad] to-[#002a7a]">
        <div className="flex items-center gap-2">
          <Upload className="h-4 w-4 text-[#ffc107] shrink-0" />
          <span className="text-sm font-semibold text-white truncate">Upload / Update Documents</span>
        </div>
      </div>

      <CardContent className={`${isMobile ? 'p-3' : 'p-5'} space-y-5 w-full overflow-x-hidden`}>
        {/* ID Proof */}
        <div className="p-3 bg-[#e6f0ff] rounded-xl border border-[#004aad]/20 w-full overflow-hidden">
          <p className="text-xs font-bold text-[#004aad] mb-3 flex items-center gap-1.5">
            <IdCard className="h-3.5 w-3.5 shrink-0" />Identity Proof
          </p>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-3 gap-3'}`}>
            <div className="min-w-0">
              ...
              {profile.id_proof_type && !idProofType && (
                <p className="text-[10px] text-green-600 mt-1 truncate">Current: {profile.id_proof_type}</p>
              )}
            </div>
            <div className="min-w-0">
              <label className={L}>
              ...
              {profile.id_proof_number && !idProofNumber && (
                <p className="text-[10px] text-green-600 mt-1 truncate">Current: {profile.id_proof_number}</p>
              )}
            </div>
            <div className="min-w-0">
              <label className={L}>Upload New File (Optional)</label>
              <div
                className="flex items-center gap-2 h-8 px-3 border border-dashed border-[#004aad]/40 rounded-md bg-white cursor-pointer hover:bg-[#e6f0ff] transition-colors w-full overflow-hidden"
                onClick={() => idRef.current?.click()}
              >
                <Upload className="h-3.5 w-3.5 text-[#004aad] shrink-0" />
                <span className="text-xs text-slate-500 truncate flex-1">
                  {idProofFile ? idProofFile.name : 'Choose file to replace...'}
                </span>
                {idProofFile && (
                  <button type="button" onClick={e => { e.stopPropagation(); setIdProofFile(null); }} className="ml-auto shrink-0">
        ...
        {/* Address Proof */}
        <div className="p-3 bg-[#fff9e6] rounded-xl border border-[#ffc107]/30 w-full overflow-hidden">
          <p className="text-xs font-bold text-[#004aad] mb-3 flex items-center gap-1.5">
            <Home className="h-3.5 w-3.5 text-[#ffc107] shrink-0" />Address Proof
          </p>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-3 gap-3'}`}>
            <div className="min-w-0">
              ...
              {profile.address_proof_type && !addressProofType && (
                <p className="text-[10px] text-green-600 mt-1 truncate">Current: {profile.address_proof_type}</p>
              )}
            </div>
            <div className="min-w-0">
              ...
              {profile.address_proof_number && !addressProofNumber && (
                <p className="text-[10px] text-green-600 mt-1 truncate">Current: {profile.address_proof_number}</p>
              )}
            </div>
            <div className="min-w-0">
              <label className={L}>Upload New File (Optional)</label>
              <div
                className="flex items-center gap-2 h-8 px-3 border border-dashed border-[#ffc107]/50 rounded-md bg-white cursor-pointer hover:bg-[#fff9e6] transition-colors w-full overflow-hidden"
                onClick={() => addrRef.current?.click()}
              >
                <Upload className="h-3.5 w-3.5 text-[#ffc107] shrink-0" />
                <span className="text-xs text-slate-500 truncate flex-1">
                  {addressProofFile ? addressProofFile.name : 'Choose file to replace...'}
                </span>
                {addressProofFile && (
                  <button type="button" onClick={e => { e.stopPropagation(); setAddressProofFile(null); }} className="ml-auto shrink-0">
                    <X className="h-3 w-3 text-red-400" />
                  </button>
                )}
              </div>
        ...
        {/* Photo */}
        <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 w-full overflow-hidden">
          <p className="text-xs font-bold text-slate-600 mb-3 flex items-center gap-1.5">
            <Camera className="h-3.5 w-3.5 shrink-0" />Photograph
          </p>
          ...
          <div
            className="flex items-center gap-2 h-8 px-3 border border-dashed border-slate-300 rounded-md bg-white cursor-pointer hover:bg-slate-50 transition-colors w-full max-w-full overflow-hidden"
          ...
          >
            ...
            {photoFile && (
              <button type="button" onClick={e => { e.stopPropagation(); setPhotoFile(null); }} className="ml-auto shrink-0">
                <X className="h-3 w-3 text-red-400" />
              </button>
            )}
          </div>
        ...
        <div className="p-3 bg-rose-50 rounded-xl border border-rose-200 w-full overflow-hidden">
          <button
            type="button"
          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-sm font-semibold text-rose-700 truncate">Partner Documents</span>
              {(profile.partner_full_name || partnerDocs.id_proof_type || partnerDocs.address_proof_type) && (
                <Badge className="bg-rose-200 text-rose-700 text-[9px] px-1.5 shrink-0">Added</Badge>
              )}
            </div>
            {partnerOpen ? (
              <ChevronUp className="h-4 w-4 text-rose-500 shrink-0" />
            ) : (
              <ChevronDown className="h-4 w-4 text-rose-500 shrink-0" />
            )}
          </button>
          ...
          {partnerOpen && (
            <div className="mt-3 space-y-3">
              {profile.partner_full_name && (
                <div className="text-xs text-rose-600 bg-rose-100 p-2 rounded-lg truncate">
              ...
              <div>
                ...
                >
                  <SelectTrigger className="h-8 text-xs border-rose-200 focus:border-rose-400 w-full">
                ...
                {(profile as any).partner_id_proof_type && !partnerDocs.id_proof_type && (
                  <p className="text-[10px] text-green-600 mt-1 truncate">Current: {(profile as any).partner_id_proof_type}</p>
                )}
                ...
                {partnerDocs.id_proof_type && (
                  <div className="mt-2">
                    ...
                    {(profile as any).partner_id_proof_number && !partnerDocs.id_proof_number && (
                      <p className="text-[10px] text-green-600 mt-1 truncate">Current: {(profile as any).partner_id_proof_number}</p>
                    )}
                  </div>
                )}
                ...
                <div className="mt-2">
                  <div
                    className="flex items-center gap-2 h-8 px-3 border border-dashed border-rose-300 rounded-md bg-white cursor-pointer hover:bg-rose-50 w-full overflow-hidden"
                  ...
                  >
                    ...
                    {partnerDocs.id_proof_file && (
                      <button type="button" onClick={e => { e.stopPropagation(); setPartnerDocs(p => ({ ...p, id_proof_file: null })); }} className="ml-auto shrink-0">
                        <X className="h-3 w-3 text-red-400" />
                      </button>
                    )}
                  </div>
              ...
              <div>
                ...
                {(profile as any).partner_address_proof_type && !partnerDocs.address_proof_type && (
                  <p className="text-[10px] text-green-600 mt-1 truncate">Current: {(profile as any).partner_address_proof_type}</p>
                )}
                ...
                {partnerDocs.address_proof_type && (
                  <div className="mt-2">
                    ...
                    {(profile as any).partner_address_proof_number && !partnerDocs.address_proof_number && (
                      <p className="text-[10px] text-green-600 mt-1 truncate">Current: {(profile as any).partner_address_proof_number}</p>
                    )}
                  </div>
                )}
                ...
                <div className="mt-2">
                  ...
                  >
                    ...
                    {partnerDocs.address_proof_file && (
                      <button type="button" onClick={e => { e.stopPropagation(); setPartnerDocs(p => ({ ...p, address_proof_file: null })); }} className="ml-auto shrink-0">
                        <X className="h-3 w-3 text-red-400" />
                      </button>
                    )}
                  </div>
              ...
              <div>
                ...
                >
                  <Camera className="h-3.5 w-3.5 text-rose-500 shrink-0" />
                  <span className="text-xs text-slate-500 truncate flex-1">
                  ...
                  {partnerDocs.photo_file && (
                    <button type="button" onClick={e => { e.stopPropagation(); setPartnerDocs(p => ({ ...p, photo_file: null })); }} className="ml-auto shrink-0">
                      <X className="h-3 w-3 text-red-400" />
                    </button>
                  )}
                </div>
        ...
        <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 w-full overflow-hidden">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <p className="text-xs font-bold text-slate-600 flex items-center gap-1.5">
              <FileCheck className="h-3.5 w-3.5 shrink-0" />Additional Documents
            </p>
            <Button
              type="button"
              variant="outline"
              size="sm"
              className="h-7 text-[10px] px-2 border-[#004aad]/20 text-[#004aad] shrink-0"
          ...
          {additionalFiles.length > 0 ? (
            <div className="space-y-1.5">
              {additionalFiles.map((f, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-white rounded-lg border border-slate-200 overflow-hidden flex-wrap gap-2">
                  ...
                  <button type="button" onClick={() => setAdditionalFiles(p => p.filter((_,j)=>j!==i))} className="text-red-400 hover:text-red-600 shrink-0">
...
export default function DocumentsTab({
  ...
  return (
    <div className={`space-y-4 w-full max-w-full overflow-x-hidden ${isMobile ? 'pb-4 px-0' : 'pb-6'}`}>
      ...
      <Card className="border border-slate-200 shadow-sm overflow-hidden w-full">
        <CardHeader className={`${isMobile ? 'px-2 py-2' : 'px-2 pt-2'} bg-gradient-to-r from-[#e6f0ff] to-[#f0f5ff] border-b border-[#004aad]/20`}>
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <div className="p-2 bg-gradient-to-br from-[#004aad] to-[#002a7a] rounded-xl shadow-md shrink-0">
                <FileCheck className="h-4 w-4 text-white" />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-slate-800 truncate">Additional Documents</h3>
                <p className="text-xs text-slate-400 mt-0.5 truncate">Other supporting documents</p>
              </div>
            </div>
            <StatusBadge status={additionalDocs.length > 0 ? 'available' : 'pending'} />
          </div>
        </CardHeader>
        <CardContent className={`${isMobile ? 'px-4 py-4' : 'p-6'} w-full overflow-x-hidden`}>
          {additionalDocs.length > 0 ? (
            <div className="space-y-4">
              <div className={`grid ${isMobile ? 'grid-cols-1 gap-3' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'}`}>
                {additionalDocs.map((doc, i) => (
                  <div key={i} className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-all duration-200 w-full">
                    <div className="p-4">
                      <div className="flex items-start gap-3">
                        <div className="p-2.5 bg-gradient-to-br from-slate-50 to-white rounded-xl border border-slate-200 shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {doc.uploaded_at && (
                            <div className="flex items-center gap-1.5 text-[10px] text-slate-400 mt-1">
                              ...
                              <span className="truncate">Uploaded: {format(parseISO(doc.uploaded_at), 'dd MMM yyyy')}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <div className={`flex ${isMobile ? 'flex-col gap-2' : 'flex-row gap-2'} mt-4`}>
      ...
      {/* ── Summary ── */}
      <Card className="bg-gradient-to-br from-[#e6f0ff] to-[#f0f5ff] border border-[#004aad]/20 w-full">
        <CardContent className="p-2">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <FileCheck className="h-4 w-4 text-[#004aad] shrink-0" />
              <span className="text-xs font-medium text-[#004aad]">Document Summary</span>
            </div>
            <Badge className="bg-[#004aad] text-white border-none text-[10px] px-2 py-0.5 shrink-0">
              {[profile.id_proof_url?1:0, profile.address_proof_url?1:0, additionalDocs.length].reduce((a,b)=>a+b,0)} Total
            </Badge>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <div className="text-center min-w-0">
              <div className={`w-8 h-8 mx-auto rounded-xl flex items-center justify-center mb-1 ${profile.id_proof_url ? 'bg-[#004aad]' : 'bg-white'}`}>
              ...
              {profile.id_proof_type && <p className="text-[7px] text-slate-400 truncate">{profile.id_proof_type}</p>}
            </div>
            <div className="text-center min-w-0">
              <div className={`w-8 h-8 mx-auto rounded-xl flex items-center justify-center mb-1 ${profile.address_proof_url ? 'bg-[#ffc107]' : 'bg-white'}`}>
              ...
              {profile.address_proof_type && <p className="text-[7px] text-slate-400 truncate">{profile.address_proof_type}</p>}
            </div>
            <div className="text-center min-w-0">
              <div className={`w-8 h-8 mx-auto rounded-xl flex items-center justify-center mb-1 ${additionalDocs.length > 0 ? 'bg-[#004aad]' : 'bg-white'}`}>
