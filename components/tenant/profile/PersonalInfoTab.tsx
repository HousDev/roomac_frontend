...
const F = 'h-8 text-[10px] rounded-md border-slate-200 focus:border-[#004aad]';
const F_SMALL = 'h-8 text-[9px] rounded-md border-slate-200 focus:border-[#004aad] px-1';  const L = 'text-[9px] font-medium text-slate-400 uppercase tracking-wider mb-1';
const SALUTATION_OPTIONS = ['Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.'];
const COUNTRY_CODE_OPTIONS = ['+91', '+1', '+44', '+61', '+971', '+65'];
  ...
  {/* Row 1: Salutation + Full Name + Country Code + Phone — all in one row */}
  <div className="col-span-2">
<div className="grid grid-cols-[45px_2fr_45px_100px] gap-2 items-end">      <div>
        <p className={L}>Title</p>
        {editing ? (
          <Select value={partnerData.salutation || ''} onValueChange={v => onPartnerFieldChange('salutation', v)} disabled={loading}>
<SelectTrigger className={`${F_SMALL} w-full`}><SelectValue placeholder="—" /></SelectTrigger>         
   <SelectContent>{SALUTATION_OPTIONS.map(o => <SelectItem key={o} value={o} className="text-xs">{o}</SelectItem>)}</SelectContent>
          </Select>
        ) : (
          <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.salutation || '—'}</p>
        )}
      </div>
      <div>
        ...
        {editing ? (
          ...
          <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.full_name || '—'}</p>
        )}
      </div>
      <div>
        <p className={L}>Code</p>
        {editing ? (
          <Select value={partnerData.country_code || '+91'} onValueChange={v => onPartnerFieldChange('country_code', v)} disabled={loading}>
<SelectTrigger className={`${F_SMALL} w-full`}><SelectValue placeholder="+91" /></SelectTrigger>            <SelectContent>{COUNTRY_CODE_OPTIONS.map(o => <SelectItem key={o} value={o} className="text-xs">{o}</SelectItem>)}</SelectContent>
          </Select>
        ) : (
          <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.country_code || '+91'}</p>
        )}
      </div>
      <div>
        ...
        {editing ? (
          ...
          <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.phone || '—'}</p>
        )}
      </div>
    </div>
  </div>
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.email || '—'}</p>
              )}
            </div>
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.gender || '—'}</p>
              )}
            </div>
            <div>
...
<p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">
  {partnerData.date_of_birth 
    ? (() => { try { return format(parseISO(partnerData.date_of_birth.substring(0,10) + 'T12:00:00'), 'dd MMM yyyy'); } catch { return partnerData.date_of_birth; } }
)()
    : '—'}
</p>              )}
            </div>
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.relationship || '—'}</p>
              )}
            </div>
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.occupation || '—'}</p>
              )}
            </div>
            <div>
              ...
              {editing ? (
                ...
                <p className="text-sm  text-slate-800 bg-white px-3 py-2 rounded-lg border border-slate-100">{partnerData.organization || '—'}</p>
              )}
            </div>
          </div>
          <div>
...
const displayDob = profile.date_of_birth
  ? (() => { try { return format(parseISO(profile.date_of_birth.substring(0,10) + 'T12:00:00'), 'dd MMM yyyy'); } catch { return profile.date_of_birth; } })()
  : '—';
  ...
  const defaultPartner: PartnerFormData = {
      salutation: (profile as any).partner_salutation || '',   // ADD

    full_name: (profile as any).partner_full_name || '',
      country_code: (profile as any).partner_country_code || '+91',  // ADD
