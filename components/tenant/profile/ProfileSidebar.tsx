...
export default function ProfileSidebar({ 
  ...
  const content = (
    <>
      ...
      <div className="text-center mb-6">
        <Avatar className={`${isMobile ? 'h-24 w-24' : 'h-28 w-28 md:h-32 md:w-32'} mx-auto mb-4`}>
          <AvatarImage src={profile.photo_url || ""} alt={profile.full_name} />
          <AvatarFallback className="text-2xl bg-gradient-to-br from-blue-100 to-purple-100">
            {profile.full_name
              ? profile.full_name
        ...
        <h2 className={`${isMobile ? 'text-xl' : 'text-xl md:text-2xl'} font-bold mb-2`}>{profile.full_name}</h2>
        <div className="flex flex-wrap gap-2 justify-center">
          <Badge variant={profile.is_active ? "default" : "secondary"}>
            {profile.is_active ? "ACTIVE" : "INACTIVE"}
          </Badge>
