...
export interface ReportFilters {
  reportType: 'revenue' | 'occupancy' | 'payments' | 'tenants' | 'expenses' | 'maintenance'| 'custom' | 'pg_revenue' | "property_payment" | "tenant_payment";
  startDate: string;
  endDate: string;
...
export async function getReportData(reportType: string, filters: { startDate?: string; endDate?: string; propertyId?: string; dateType?: string }): Promise<any> {
  try {
    const params = new URLSearchParams();
    if (filters.startDate) params.append('startDate', filters.startDate);
    if (filters.endDate) params.append('endDate', filters.endDate);
