...
// Get complaint categories - values from master_item "Complaint Category"
export const getComplaintCategories = async (): Promise<ComplaintCategory[]> => {
  try {
    // Fetch values for master_item type "Complaint Category"
    const res = await consumeMasters({ tab: 'Requests', type: 'Complaint Category' });
    
    console.log('🔍 Complaint Categories response:', res);
    
    if (res?.success && Array.isArray(res.data) && res.data.length > 0) {
      return res.data.map((item: any) => ({
        id: item.value_id,      // master_item_value id
        code: (item.value_name || '').toLowerCase().replace(/\s+/g, '_'),
        name: item.value_name,
        tab: item.tab_name || 'Requests',
...
// Get complaint reasons - ALL values from master_item "Complaint Reason"
export const getComplaintReasons = async (): Promise<ComplaintReason[]> => {
  try {
    // Fetch ALL values for master_item type "Complaint Reason"
    const res = await consumeMasters({ tab: 'Requests', type: 'Complaint Reason' });
    
    console.log('🔍 Complaint Reasons response:', res);
    ...
    console.warn('⚠️ No complaint reasons from API, using fallback');
  } catch (error) {
    console.error('❌ getComplaintReasons error:', error);
  }
};
