...
// ─── Print via hidden iframe (same tab, no new window) ───────────────────────
function printHTMLInPlace(html: string) {
  let iframe = document.getElementById("__tenant_print_frame") as HTMLIFrameElement | null;
  if (!iframe) {
    iframe = document.createElement("iframe");
    iframe.id = "__tenant_print_frame";
...
<div class="footer"><span>${orgName}</span><span>Generated ${new Date().toLocaleDateString("en-IN",{day:"2-digit",month:"long",year:"numeric"})}</span></div>
</body></html>`;
};

const handlePrintProfile = () => {
  printHTMLInPlace(buildOverviewPrintHTML());
};
...
const doPrint = (stay: any) => {
    const html = `<!DOCTYPE html><html><head><title>Stay #${stay.stayNumber}</title>
    ...
    <div class="ft">${orgName}</div></body></html>`;
    printHTMLInPlace(html);
  };
...
const doPrintAll = () => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Stay History · ${tenant.full_name}</title>
...
// ─── ID Card Modal ────────────────────────────────────────────────────────────
function TenantIdCard({ tenant, assignment, onClose, orgLogo, orgName }: { tenant: any; assignment: any; onClose: () => void; orgLogo?: string; orgName?: string }) {
  const brandName = orgName || "Roomac Co-Living";
...
const handlePrint = () => {
  printHTMLInPlace(buildCardHTML());
};
