  ...
  const fetchReportDetails = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (activeTab === "tenant") {
          // Calculation parameters are calculated backend-side on demand
        }
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 p-4 md:p-2  space-y-4 relative overflow-hidden font-sans">
                          ...
                          <>
                            <td className="p-3">
                              <div className="space-y-0.5">
                                {renderTenantNameWithAvatar(row.full_name)}
                                {row.is_couple_booking && !row.bed_number && (
                                  <div className="text-[10px] text-pink-600 font-medium italic">
                                    Shared with {row.is_vacated ? row.vacate_records?.[0]?.partner_full_name : row.partner_full_name || "partner"}
                                  </div>
                                )}
                              </div>
                            </td>
                            ...
                            <td className="p-3 text-slate-600 font-bold">
                              {row.is_couple_booking && !row.bed_number ? "Shared" : row.bed_number || "—"}
                            </td>
                            <td className="p-3 text-right font-mono font-bold text-slate-800 dark:text-slate-100">
                              {row.is_couple_booking && !row.bed_number ? (
                                <span className="text-[10px] text-slate-400 font-normal italic">Shared</span>
                              ) : (
      ...
      {sidebarOpen && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-[40] w-80 bg-white dark:bg-slate-900 shadow-2xl border-l border-slate-200 dark:border-slate-800 flex flex-col justify-between transition-all duration-300 ease-in-out transform translate-x-0 animate-in slide-in-from-right duration-200 print:hidden">
            
            <div>
              {/* Header: Solid Deep Blue with white text, Close button */}
              <div className="flex items-center justify-between bg-gradient-to-r from-[#0A1F5C] via-[#123A9A] to-[#1E4ED8] p-4 text-white">
                <div className="flex items-center gap-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={tempAssignedStaff} onValueChange={setTempAssignedStaff}>
                        ...
                        <SelectContent>
                          ...
                          {Array.from(new Set(reportData.map(r => r.staff_name).filter(Boolean))).map((staff: any) => (
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={tempState} onValueChange={setTempState}>
                        ...
                        <SelectContent>
                          ...
                          {Array.from(new Set(reportData.map(r => r.state).filter(Boolean))).map((s: any) => (
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/40 flex items-center justify-between gap-3">
              <Button
                ...
                className="flex-1 border-slate-200 text-slate-700 bg-white hover:bg-slate-100 text-xs h-9 rounded-md flex items-center justify-center gap-1.5 font-semibold"
              >
