...
// ─── PhonePe Card (standalone, no TabsContent) ───────────────────────────────
function PhonePeCardOnly({ settings, updateSetting, showSecrets, toggleSecretVisibility, onSave }: CardOnlyProps) {
  const [modalOpen, setModalOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [draft, setDraft] = useState<Record<string, any>>({});
    ...
    <>
      <Card className="border border-slate-200 shadow-md rounded-2xl overflow-hidden w-full md:w-[380px] h-auto md:h-[380px] flex flex-col">
        <CardContent className="p-5 flex-1 flex flex-col">
          ...
          <div className="mt-auto pt-3 border-t border-slate-100">
            <Button onClick={openModal} className="w-full bg-gradient-to-r from-[#0A1F5C] via-[#123A9A] to-[#1E4ED8] hover:bg-blue-700 text-white rounded-xl h-10 text-sm"><Settings2 className="h-4 w-4 mr-2" />Configure</Button>
            <div className="flex justify-between items-center mt-3">
              <span className="text-xs text-slate-400">Status</span>
              <span className={`px-2.5 py-1 rounded-full font-semibold border text-xs ${isConnected ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>{isConnected ? '● Connected' : '○ Disconnected'}</span>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-3">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div className="p-4 space-y-3 max-h-[70vh] overflow-y-auto">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-2.5 flex items-start gap-2"><Shield className="h-3.5 w-3.5 text-purple-600 mt-0.5 shrink-0" /><div className="text-[11px] text-purple-700 space-y-0.5"><p className="font-semibold text-purple-800">Get Started with PhonePe</p><p>• Sign up at phonepe.com/business</p><p>• Get credentials from Dashboard → API Keys</p><p>• Use UAT credentials for testing</p></div></div>
            </div>
            <div className="px-4 pb-4 flex gap-2 justify-end border-t border-slate-100 pt-3">
              <Button variant="outline" onClick={closeModal} disabled={saving} className="h-9 text-xs px-4 border-slate-300">Cancel</Button>
