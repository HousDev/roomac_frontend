...
//         <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
...
//         <div
//           className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-full h-1 sm:h-1.5 transition-all duration-500"
//           style={{
//             width: `${Math.min(
//               (stats.daysUntilRentDue / 30) * 100,
//               100
//             )}%`,
//           }}
//         />
...
//           size="sm"
//           variant="outline"
//           className="w-full h-7 text-xs border-purple-200 hover:bg-purple-50 hover:text-black"
...
export default function TenantPortalPage() {
  ...
  const fetchNotifications = useCallback(async (showLoading = false) => {
    try {
      ...
      console.log('📋 Fetched notifications:', formattedNotifs);
      setNotifications(formattedNotifs);
      setUnreadCount(count);
      setStats(prev => ({ ...prev, unreadNotifications: count }));
    } catch (error) {
...
console.log('🔍 CONTRACT FIELDS FROM BACKEND:', {
  ...
  lockin_period_months: d.lockin_period_months,
  ...
  property_manager_email: propertyRes.data.property_manager_email,
  // ← ADD these two lines:
  amenities: prev?.amenities || propertyRes.data.amenities || null,
...
const getAmenitiesDisplay = () => {
  ...
  const rentAmount = tenant?.rent_per_bed || tenant?.monthly_rent || stats.monthlyRent;
    ...
    <div className="p-2 sm:p-2">
          ...
          <CardContent className="p-2 lg:p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 lg:gap-2">
                ...
                <div>
                  <p className="text-[10px] lg:text-xs font-medium text-slate-600 leading-tight">Rent Due</p>
                  <div className="flex items-baseline gap-0.5">
                    <p className="text-sm lg:text-lg font-bold text-slate-900 leading-tight">
                      {stats.daysUntilRentDue}
                    </p>
                    <span className="text-[9px] lg:text-xs font-medium text-slate-500">days</span>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] lg:text-xs text-slate-500 mb-0.5">Amount</p>
                {/* ← Real rent from backend */}
          ...
          <CardContent className="p-2 lg:p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 lg:gap-2">
                ...
                <div>
                  <p className="text-[10px] lg:text-xs font-medium text-slate-600 leading-tight">Open Issues</p>
                  <p className="text-sm lg:text-lg font-bold text-slate-900 leading-tight">{stats.openComplaints}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] lg:text-xs text-slate-500 mb-0.5 lg:mb-1">Urgent</p>
                <Badge variant="destructive" className="text-[9px] lg:text-xs px-1.5 py-0 h-4 lg:h-auto">
                  {stats.urgentComplaints}
                </Badge>
              </div>
            </div>
            <div className="mt-1.5 lg:mt-3 flex gap-1">
              <div className="flex-1 bg-amber-100 rounded-lg p-1 lg:p-1.5 text-center">
                <p className="text-[9px] lg:text-xs font-semibold text-amber-900 leading-tight">In Progress</p>
                <p className="text-[9px] lg:text-xs text-amber-700">{stats.inProgressComplaints}</p>
              </div>
              <div className="flex-1 bg-orange-100 rounded-lg p-1 lg:p-1.5 text-center">
                <p className="text-[9px] lg:text-xs font-semibold text-orange-900 leading-tight">Pending</p>
          ...
          <CardContent className="p-2 lg:p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 lg:gap-2">
                ...
                <div>
                  <p className="text-[10px] lg:text-xs font-medium text-slate-600 leading-tight">Pending</p>
                  <p className="text-sm lg:text-lg font-bold text-slate-900 leading-tight">{stats.pendingCount}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] lg:text-xs text-slate-500 mb-0.5 lg:mb-1">Total</p>
                {/* ← Real pending amount from backend */}
                <p className="text-xs lg:text-base font-bold text-emerald-700">
                  ₹{Number(stats.totalPending || rentAmount).toLocaleString("en-IN")}
                </p>
              </div>
            </div>
            <div className="mt-1.5 lg:mt-3">
          ...
          <CardContent className="p-2 lg:p-3">
            <div className="flex items-center justify-between mb-1.5 lg:mb-2">
              <div className="flex items-center gap-1.5 lg:gap-2">
                ...
                <div>
                  <p className="text-[10px] lg:text-xs font-medium text-slate-600 leading-tight">Notifications</p>
                  <p className="text-sm lg:text-lg font-bold text-slate-900 leading-tight">{stats.unreadNotifications}</p>
                </div>
              </div>
              {stats.unreadNotifications > 0 && (
                <Badge className="bg-purple-500 hover:bg-purple-600 text-[9px] lg:text-xs px-1.5 h-4 lg:h-auto">New</Badge>
              )}
            </div>
            <div className="mt-1.5 lg:mt-3">
              <Button
      ...
      <div className="grid grid-cols-4 gap-1.5 sm:gap-3 mb-4 lg:mb-6">
        ...
        >
          ...
          <span className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-center leading-tight">Raise Complaint</span>
        ...
        >
          ...
          <span className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-center leading-tight">View Agreement</span>
          <span className="text-[8px] lg:text-[10px] text-slate-400 text-center leading-tight hidden lg:block">Rental contract</span>
        </Button>
        ...
        >
          ...
          <span className="text-[9px] sm:text-[10px] lg:text-xs font-medium text-center leading-tight">Download Invoice</span>
          <span className="text-[8px] lg:text-[10px] text-slate-400 text-center leading-tight hidden lg:block">Payment receipts</span>
        </Button>
      </div>
...
<div className="space-y-3 max-h-[43vh] overflow-y-auto pr-1">                      {notifications.map((n) => (
                        <div 
                          key={n.id} 
className={`p-2.5 sm:p-3 rounded-md border cursor-pointer transition-all hover:shadow-sm ${                            !n.is_read ? "bg-blue-50 border-blue-200" : "bg-white border-slate-200"
                          }`}
                          onClick={() => handleNotificationClick(n)}
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-start justify-between">
                          <p className="font-medium text-xs sm:text-sm text-slate-900 leading-tight">{n.title}</p>
                                {!n.is_read && <div className="h-2 w-2 bg-blue-600 rounded-full mt-1" />}
                              </div>
                ...
                {[
                  {
                    ...
                    value: tenant?.check_in_date ? formatDate(tenant.check_in_date) : "—",
                  },
                  {
                    label: "Lock-in",
  ...
  <DialogContent className="sm:max-w-md w-[94%] p-0 overflow-hidden rounded-xl">
    ...
    <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4">
      ...
      <div>
        <Label className="text-[11px] sm:text-sm font-medium">Title *</Label>
        <Input
      ...
      <div>
        <Label className="text-[11px] sm:text-sm font-medium">Category *</Label>
        <Select
      ...
      {complaintReasons.length > 0 && (
        <div>
          <Label className="text-[11px] sm:text-sm font-medium">Reason</Label>
          <Select
      ...
      <div>
        <Label className="text-[11px] sm:text-sm font-medium">Priority</Label>
        <Select
      ...
      <div>
        <Label className="text-[11px] sm:text-sm font-medium">Description *</Label>
        <Textarea
  ...
  <DialogContent className="sm:max-w-2xl w-[96%] p-0 overflow-hidden rounded-xl">
    ...
    <div className="px-4 py-3 space-y-2.5">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div>
          ...
          <Select
            ...
            onValueChange={(v) => setLeaveRequest({ ...leaveRequest, leave_type: v })}
          >
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <Label className="text-[11px] font-medium">Contact Address (During Leave)</Label>
          <Input
