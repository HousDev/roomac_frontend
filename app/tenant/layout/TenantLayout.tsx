...
//             </p>
//           </div>
//           {(tenant?.is_active === 1 || tenant?.is_active === false) ? (
...
function NotificationPopup({
  ...
  return (
    <div
      ref={popupRef}
      className="absolute right-0 mt-2 w-72 sm:w-80 md:w-96 bg-white rounded-lg shadow-xl border border-slate-200 z-50"
    >
      <div className="p-3 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell className="h-4 w-4 text-blue-600" />
...
// Backend may return is_active as boolean true/false or number 1/0
function isTenantActive(tenant: TenantProfile | null): boolean {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-1 relative z-10">
          <div className="flex items-center gap-1">
            ...
            <p className="text-[8px] lg:text-[10px] font-semibold uppercase tracking-widest text-blue-100">
              Your Accommodation
            </p>
          </div>
          {isActive ? (
