...
// Welcome back, <span className="text-[#0149ab]">
//   {tenant?.salutation ? `${tenant.salutation} ${tenant?.full_name?.split(" ")[0]}` : tenant?.full_name?.split(" ")[0] || "Tenant"}
// </span>            </h1>
...
//     .then((res: any) => { 
//       if (res?.success) {
//         console.log("🔍 Salutation:", res.data?.salutation);
            ...
            {profileOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-slate-200 z-50">
                ...
                <div className="p-1">
                  <Button
                    variant="ghost"
                    className="w-full justify-start h-9 px-2 text-sm hover:bg-slate-100"
                    onClick={() => { onNavigate("/tenant/profile"); setProfileOpen(false); }}
                  >
                    <User className="h-4 w-4 mr-2" /> Profile
                  </Button>
                ...
                <div className="border-t border-slate-200 p-1">
                  <Button
                    variant="ghost"
                    className="w-full justify-start h-9 px-2 text-sm text-red-600 hover:bg-red-50"
                    onClick={() => { onLogout(); setProfileOpen(false); }}
                  >
                    <LogOut className="h-4 w-4 mr-2" /> Logout
                  </Button>
