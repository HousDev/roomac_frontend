...
// Assets.tsx — Full Inventory Management Page
// Theme: Matches EnquiryForm style + Payment stats cards + Room filter sidebar
...
//   deleteInventory,
//   getInventoryStats,
// } from "@/lib/assestsApi";import { getMasterItemsByTab, getMasterValues } from "@/lib/masterApi";
...
// // ─── Style tokens (matches EnquiryForm) ──────────────────────────────────────
...
// // ─── Section header (matches EnquiryForm) ─────────────────────────────────────
...
// // ─── Stat Card (matches Payment page) ─────────────────────────────────────────
...
//   // Load master categories from Properties > Category master item
//  const loadCategories = useCallback(async () => {
//   try {
//     const res = await getMasterItemsByTab('Properties');
//     console.log('Tab response:', res);

//     // res.data directly array hai based on masterController
//     const list = Array.isArray(res.data) ? res.data : [];
//     const catItem = list.find((i: any) => i.name?.toLowerCase() === 'category');
//     console.log('catItem:', catItem);

//     if (!catItem) {
//       console.warn('Category master item not found in Properties tab');
//       return;
//     }

//     // masterController getValuesByItemId uses master_item_id param
//     const vRes = await getMasterValues(catItem.id);
//     console.log('Values raw response:', JSON.stringify(vRes));
...
//         .map((v: any) => ({
//           id:   String(v.id),
//           name: v.value || v.name || '',  // master_values has 'value' column
//         }))
//     );
//   } catch (err) {
...
//     console.log('Properties full response:', JSON.stringify(res));

//     // Try all possible paths
//     const list =
//       res?.data?.data ||   // { data: { data: [...] } }
//       res?.data ||         // { data: [...] }
//       (res as any)?.results ||  // { results: [...] }
//       [];

//     const arr = Array.isArray(list) ? list : Object.values(list);
//     console.log('Properties array:', arr);
...
//   // ── Filtered + column-searched items ────────────────────────────────────────
...
//   const clearFilters = () => {
//     setStockFilter('all'); setCategoryFilter('all'); setPropertyFilter('all');
//   };
...
//       {/* ── Header ────────────────────────────────────────────────────────── */}
//       <div className="sticky top-0 z-20 bg-white border-b px-4 py-3 flex items-center justify-between gap-2">
//         <div>
//           <h1 className="text-base font-bold text-gray-800">Inventory Assets</h1>
//           <p className="text-[10px] text-gray-500">Manage all inventory items</p>
//         </div>
//         <div className="flex items-center gap-2">
//           <Button variant="outline" size="sm" className="h-8 text-xs"
//             onClick={() => setSidebarOpen(o => !o)}>
//             <Filter className="h-3.5 w-3.5 mr-1" />
//             Filters {hasFilters && <span className="ml-1 text-blue-600 font-bold">•</span>}
//           </Button>
//           <Button variant="outline" size="sm" className="h-8 text-xs" onClick={handleExport}>
//             <Download className="h-3.5 w-3.5 mr-1" />
//             Export
//           </Button>
//           <Button variant="outline" size="sm" className="h-8 text-xs" onClick={loadAll} disabled={loading}>
//             <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} />
//           </Button>
//           <Button size="sm" className="h-8 text-xs bg-gradient-to-r from-blue-600 to-indigo-600 text-white" onClick={openAdd}>
...
//         {/* ── Filter Sidebar (like RoomFilters) ─────────────────────────── */}
//         {sidebarOpen && (
//           <aside className="w-56 shrink-0 bg-white border-r min-h-screen p-3 sticky top-[57px] h-[calc(100vh-57px)] overflow-y-auto">
//             <div className="flex items-center justify-between mb-3">
//               <span className="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Filters</span>
//               <div className="flex gap-1">
//                 {hasFilters && (
//                   <button onClick={clearFilters} className="text-[9px] text-blue-600 font-semibold">Clear</button>
//                 )}
...
//             {/* Stock Status */}
//             <div className="mb-4">
//               <p className="text-[10px] font-bold text-gray-500 uppercase mb-1.5">Stock Status</p>
//               {(['all', 'low_stock', 'out_of_stock'] as StockFilter[]).map(s => (
//                 <label key={s} className="flex items-center gap-2 py-1 cursor-pointer">
//                   <input type="radio" name="stock" value={s}
...
//                     {s === 'all' ? 'All Items' : s === 'low_stock' ? 'Low Stock' : 'Out of Stock'}
//                   </span>
//                 </label>
//               ))}
//             </div>
...
//             {/* Category */}
//             <div className="mb-4">
//               <p className="text-[10px] font-bold text-gray-500 uppercase mb-1.5">Category</p>
...
//                   className="accent-blue-600 h-3 w-3"
//                 />
//                 <span className="text-[11px] text-gray-700">All Categories</span>
//               </label>
//               {categories.map(c => (
//                 <label key={c.id} className="flex items-center gap-2 py-1 cursor-pointer">
//                   <input type="radio" name="cat" value={String(c.id)}
//                     checked={categoryFilter === String(c.id)}
//                     onChange={() => setCategoryFilter(String(c.id))}
//                     className="accent-blue-600 h-3 w-3"
//                   />
...
//             {/* Property */}
//             <div className="mb-4">
//               <p className="text-[10px] font-bold text-gray-500 uppercase mb-1.5">Property</p>
//               <label className="flex items-center gap-2 py-1 cursor-pointer">
//                 <input type="radio" name="prop" value="all"
//                   checked={propertyFilter === 'all'}
//                   onChange={() => setPropertyFilter('all')}
//                   className="accent-blue-600 h-3 w-3"
//                 />
//                 <span className="text-[11px] text-gray-700">All Properties</span>
//               </label>
//               {properties.map(p => (
//                 <label key={p.id} className="flex items-center gap-2 py-1 cursor-pointer">
//                   <input type="radio" name="prop" value={String(p.id)}
//                     checked={propertyFilter === String(p.id)}
//                     onChange={() => setPropertyFilter(String(p.id))}
//                     className="accent-blue-600 h-3 w-3"
//                   />
...
//         {/* ── Main Content ──────────────────────────────────────────────── */}
//         <main className="flex-1 p-3 sm:p-4 space-y-3 min-w-0">

//           {/* Stats Cards — Payment style */}
//           <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-2 sticky top-[57px] z-10">
...
//                         { key: null,        ph: '' },   // total value — no filter
...
//                               item.quantity <= item.min_stock_level ? 'text-orange-600' :
//                               'text-gray-800'
//                             }>
//                               {item.quantity}
//                             </span>
...
//       {/* ══ Add / Edit Dialog (EnquiryForm style) ═══════════════════════════ */}
